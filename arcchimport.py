...
def list_ecs_services(ecs, cluster):
    ...
    while "nextToken" in result:
        result = ecs.list_services(cluster=cluster, nextToken = result["nextToken"])
        if "serviceArns" in result:
            all_services.extend(result.get("serviceArns"))
    return all_services
...
def list_ecs_tasks(ecs, cluster):
    ...
    while "nextToken" in result:
        result = ecs.list_tasks(cluster=cluster, nextToken = result["nextToken"])
        if "taskArns" in result:
            all_tasks.extend(result.get("taskArns"))
    return all_tasks
...
def list_glacier_vault_tags(glacier, vault_name):
    try:
...
def get_glacier_vaults(glacier, target_info):
    ...
    try:
        ...
        for vault in vaults:
            ...
            described_vault = glacier.describe_vault(vaultName=vault_name)
            tags = list_glacier_vault_tags(glacier, vault_name)
            tagsObj = {
                'Tags': tags
            }
            described_vault.update(tagsObj)
